...
at_exit_list() {
    [ -z "${__at_exit_handlers:-}" ] && { print "$(i18n 'AT_EXIT_NO_HANDLERS' 2>/dev/null || echo 'No at-exit handlers registered')"; return 0; }
    OLDIFS=$IFS; IFS='
'
    for l in $__at_exit_handlers; do
        print "$l"
...
__at_exit_install_from_filters() {
    ...
    [ -z "${MODPATH:-}" ] && { __at_exit_log_info "AUTO_EXTRACT_NO_MODPATH"; return 0; }
            ...
            if [ -n "$entry" ]; then
                if [ -t 0 ] && type "__confirm_install_file_do_from_zip" >/dev/null 2>&1; then
                    ...
                    if has_command "unzip"; then
                        unzip -o -j "${ZIPFILE}" "$entry" -d "$tmpdir" >/dev/null 2>&1 || { rm -rf "$tmpdir"; __at_exit_log_warn_t "AUTO_EXTRACT_INSTALL_FAILED" "$rel"; continue; }
                    else
                        unzip -p "${ZIPFILE}" "$entry" > "$tmpdir/$(basename "$entry")" 2>/dev/null || { rm -rf "$tmpdir"; __at_exit_log_warn_t "AUTO_EXTRACT_INSTALL_FAILED" "$rel"; continue; }
                    fi
