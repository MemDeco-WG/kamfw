...
'
    ...
    # Nothing to do?
    [ ! -s "$__prune_tmp" ] && { info "$(i18n 'PRUNE_ZYGISK_NO_FILES' 2>/dev/null | t "$arch" 2>/dev/null || printf 'No zygisk files to remove for arch %s' "$arch")"; rm -f "$__prune_tmp" 2>/dev/null || true; return 0; }
    ...
    while IFS= read -r f || [ -n "$f" ]; do
        [ -z "$f" ] && continue
        count=$((count + 1))
        [ "$dryrun" = true ] && { info "$(i18n 'PRUNE_ZYGISK_WOULD_REMOVE' 2>/dev/null | t "$f" 2>/dev/null || printf 'Would remove: %s' "$f")"; continue; }

        # Safety: ensure target is inside zdir
        case "$f" in
            "$zdir"/*) ;; # ok
            *) warn "$(i18n 'PRUNE_ZYGISK_REMOVE_FAILED' 2>/dev/null | t "$f" 2>/dev/null || printf 'Refusing to remove (outside dir): %s' "$f")"; continue ;;
        esac
